...
class FusedDense(nn.Linear):
    ...
    def forward(self, x, process_group=None):
        ...
        assert dtype_eligible, "dtype must be fp16 or bf16 for fused kernel"
        assert x.is_cuda, "Inputs must be on GPU"
        assert self.weight.is_cuda, "Weights must be on GPU"
        assert (self.bias is None or self.bias.is_cuda), "Biases must be on GPU, or do not exist"
