...
class TUCOREGCN_BERT(nn.Module):
    def __init__(
        self, config, num_labels, gcn_layers=2, activation="relu", gcn_dropout=0.6
    ):
        ...
        self.classifier = nn.Linear(
            config.hidden_size * 3 * (self.gcn_layers + 1), self.num_labels
        )
    ...
    def forward(
        ...
        Encoder Module
        """
        sequence_outputs, pooled_outputs = self.bert(
            input_ids, speaker_ids, token_type_ids, attention_mask
        )
        """
        Turn Attention Module
        """
        sequence_outputs, _ = self.turnAttention(
