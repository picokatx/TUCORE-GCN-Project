...
class TUCOREGCN_LlamaConfig(PretrainedConfig):
    r"""
...
class LlamaSpeakerEmbeddings(nn.Module):
    ...
    def forward(
        ...
        embeddings = self.word_embeddings(input_ids)+self.speaker_embeddings(speaker_ids)
        return embeddings
...
class TUCOREGCN_LlamaPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = TUCOREGCN_LlamaConfig
...
class TUCOREGCN_Llama(TUCOREGCN_LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class TUCOREGCN_BertForSequenceClassification(TUCOREGCN_LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config

        self.tucoregcn_bert = TUCOREGCN_Llama(config)
        classifier_dropout = (
