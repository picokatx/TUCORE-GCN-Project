...
class T5FlashAttention2(T5Attention):
    ...
    def forward(
        ...
        attn_output = self._flash_attention_forward(
            query_states, key_states, value_states, mask, query_length, dropout=dropout_rate
        )
        ...
        if output_attentions:
            print("cannot output attentions right now")
        return outputs
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = T5_ATTENTION_CLASSES[config._attn_implementation](config, has_relative_attention_bias=has_relative_attention_bias)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
